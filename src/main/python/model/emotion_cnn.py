...
class EmotionCNN:
    ...
    def backpropagate(self, image, true_label, learning_rate=0.01):
        ...
        return loss
def learn(model, train_images, train_labels, test_images, test_labels, epochs=10, learning_rate=0.01):
    """
    Trains the model for a given number of epochs.
    Returns lists of training losses and test losses recorded after each epoch.
    """
    train_losses = []
    test_losses = []
    ...
    for epoch in range(epochs):
        ...
        for i in range(len(train_images)):
            ...
            loss = model.backpropagate(image, true_label, learning_rate)
            total_train_loss += loss
        
        avg_train_loss = total_train_loss / len(train_images)
        ...
        for i in range(len(test_images)):
            ...
            # One-hot encode true label (same as in the training loop)
            num_classes = model.fc_bias.shape[0]
            one_hot_label = np.zeros(num_classes, dtype=np.float64)  # Create a zero vector for one-hot encoding
            one_hot_label[true_label] = 1  # Set the index of the true label to 1

            # Compute loss
            loss = model.cross_entropy_loss(predictions, one_hot_label)
